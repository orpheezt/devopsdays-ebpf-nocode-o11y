...
def build() -> None:
    ...
    console.print(
        f"Building container image {IMAGE_NAME}:{TAG} using Go {GO_VERSION}..."
    )
    run(
        [
            "buildah",
...
def diagnostics(
    ...
    console.print(
        f"* Compiled Go binary accounts for [yellow]{(totals['binary'] / total_size) * 100:.1f}%[/] of image size."
    )
    console.print(
